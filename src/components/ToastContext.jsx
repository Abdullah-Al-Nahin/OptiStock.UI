    ...
    <ToastContext.Provider value={{ success, error, warning }}>
      ...
      <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-4 pointer-events-none">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`animate-toast-in toast-glass flex flex-col overflow-hidden rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] border min-w-[320px] pointer-events-auto
              ${toast.type === 'success' ? 'border-[#4ade80]/35' :
                toast.type === 'error' ? 'border-[#f87171]/35' :
                'border-[#fbbf24]/35'}`}
          >
            <div className="flex items-center gap-4 px-5 py-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xl
                ${toast.type === 'success' ? 'bg-[#4ade80]/10 text-[#4ade80]' :
                  toast.type === 'error' ? 'bg-[#f87171]/10 text-[#f87171]' :
